    ...
    <tbody>
		<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><a href="<?php echo U('User/editAction?id=='.$vo['id']);?>">编辑</a>
				<a href="<?php echo U('User/setStatus?ids='.$vo['id'].'&status='.abs(1-$vo['status']));?>" class="ajax-get"><?php echo (show_status_op($vo["status"])); ?></a>
				<a href="<?php echo U('User/setStatus?status=-1&ids='.$vo['id']);?>" class="confirm ajax-get">删除</a>
                </td>
