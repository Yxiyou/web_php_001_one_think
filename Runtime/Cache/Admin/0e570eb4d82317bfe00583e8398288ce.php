...
<body>
    ...
    <tbody>
		<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><a href="<?php echo U('think/lists?model='.$vo['name']);?>">数据</a>
				<?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('AuthManager/changeStatus?method=forbidGroup&id='.$vo['id']);?>" class="ajax-get">禁用</a>
				<?php else: ?>
			...
			<a href="<?php echo U('AuthManager/modelauth?group_name='.$vo['title'].'&group_id='.$vo['id']);?>" >模型授权</a>
